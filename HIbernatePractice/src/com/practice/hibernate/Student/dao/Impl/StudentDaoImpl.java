...
public class StudentDaoImpl implements StudentDao {
	...
	public int insert(Student student) {
		...
		int res=(int)session.save(student);  //persist wont return anything but save return id of the column which get inserted
		//session.persist(student);
		session.evict(student);
		student.setSname("Ayush");
		Student student1=(Student) session.merge(student);
		System.out.println(student1);
		//session.saveOrUpdate(student);   // It wont give the exception if primary key coulmn already exist and trying to insert the entry with same id. It will simply update the database.
		...
		//int res1=(int)session.save(student);
